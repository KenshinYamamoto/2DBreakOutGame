...
public class Ball_script : MonoBehaviour 
{
    ...
    public float ballInitialVelocityY = 250f;   //The initial velocity of the Ball
	...
	private bool ballInPlay = false;	//Boolean to know if the Ball inPlay (in moving) or not
	...
	void Start () 
    {
		...
		ballInitialVelocityX = Random.Range(-ballInitialVelocityY*2, ballInitialVelocityY*2);
	}
	...
	void Update () 
    {
        ...
        if (Input.GetButtonDown("Jump") && ballInPlay == false && GameManager_script.startGame)
        {
            ...
            if(Input.GetAxis ("Horizontal") == 0f)		//Checking if the Paddle is standing still
				rb2D.AddForce(new Vector2(ballInitialVelocityX, ballInitialVelocityY));	//Adding force to the Balle
			else if(Input.GetAxis ("Horizontal") > 0f)		//Checking if the Paddle is moving right
				rb2D.AddForce(new Vector2(ballInitialVelocityY, ballInitialVelocityY));		//Adding force to the Balle
